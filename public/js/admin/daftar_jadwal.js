...
$("#btnAdd").on("click", function () {
    ...
    var materialName = $("#nama_material option:selected").text()
    var lecturerName = $("#nama_dosen option:selected").text()
    var dayName = $("#hari option:selected").text()
    var clockName = $("#jam option:selected").text()
    var classNameVal = $("#nama_kelas option:selected").val()
    var materialNameVal = $("#nama_material option:selected").val()
    var lecturerNameVal = $("#nama_dosen option:selected").val()
    var dayNameVal = $("#hari option:selected").val()
    var clockNameVal = $("#jam option:selected").val()
    ...
    $("select").each(function (i, e) {
        ...
        $("#confirmSameJadwal").attr("hidden", true)
        if ($(this).val() == '') {
            ...
            if (Object.keys(jadwalTemp).length > 0) {
                $.each(jadwalTemp, function (i, v) { 
                    if (className == v.className && materialName == v.materialName && lecturerName == v.lecturerName && dayName == v.dayName && clockName == v.clockName) {
                        $("#confirmSameJadwal").removeAttr("hidden")
                        selectValSame = false
                        return false
                    }
                })
    ...
    if (selectVal && selectValSame) {
        ...
        `)
        jadwalTemp[colNo] = {className, materialName, lecturerName, dayName, clockName}
        jadwalList[colNo] = {classNameVal, materialNameVal, lecturerNameVal, dayNameVal, clockNameVal}
        $("#jadwal_val").val(JSON.stringify(jadwalList))
        $("#"+ colNo +"_btnDelete").on("click", function () {
            var e = $(this).closest("tr")
            var numberId = parseInt(e.find(".numberId").text())

            number--
            delete jadwalTemp[numberId]
            delete jadwalList[numberId]
